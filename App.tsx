...
const AppContent: React.FC = () => {
  ...
  // זיהוי הנתיב הנוכחי לצורך עיצוב הכפתורים בתפריט
  const currentPath = location.pathname === '/' ? 'home' : location.pathname.substring(1);

  useEffect(() => {
    try {
      console.log("MyJew App initialized with Router");
    } catch (e) {
    ...
    <div className="flex flex-col min-h-screen relative" dir={direction}>
        ...
        <nav className={`mx-auto max-w-7xl md:rounded-3xl p-3 md:p-4 transition-all duration-300 ${scrolled ? 'glass-panel shadow-lg rounded-b-3xl md:rounded-3xl' : 'bg-white/60 dark:bg-black/40 backdrop-blur-md md:bg-transparent md:backdrop-blur-none border-b border-white/20 md:border-none'}`}>
          <div className="flex items-center justify-between relative">
            <div className="flex items-center gap-2">
               <Link to="/" className="hover:opacity-80 transition-opacity focus:outline-none">
                 <Logo />
               </Link>
            </div>
            ...
            <div className="hidden xl:flex items-center p-1.5 gap-1 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white/80 dark:bg-gray-900/60 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-glass rounded-full z-20">
              {navItems.map(item => (
                <Link 
                  ...
                  className={`px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 flex items-center justify-center ${currentPath === item.id ? 'bg-brand-blue text-white shadow-md' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100/50 dark:hover:bg-white/10'}`}
                >
                  {item.label}
                </Link>
              ))}
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setIsMobileMenuOpen(true)} className="xl:hidden w-10 h-10 flex items-center justify-center rounded-full glass-panel text-gray-800 dark:text-white hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 xl:hidden">
            <div className="absolute inset-0 bg-black/30 backdrop-blur-[2px] animate-fade-in" onClick={() => setIsMobileMenuOpen(false)}></div>
            <div className="absolute top-0 right-0 bottom-0 w-[85%] max-w-sm bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl p-6 border-l border-white/20 animate-fade-in-up h-full flex flex-col">
                ...
                <div className="flex flex-col gap-3 overflow-y-auto flex-grow pb-8">
                    {navItems.map(item => (
                        <Link
                            key={item.id}
                            to={item.id === 'home' ? '/' : `/${item.id}`}
                            onClick={() => { setIsMobileMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                            className={`p-4 rounded-2xl flex items-center gap-4 text-right transition-all font-bold text-lg ${currentPath === item.id ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-gray-50 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300'}`}
                        >
      ...
      <main className="flex-grow p-4 pt-4 md:pt-4">
        <Suspense fallback={<Spinner />}>
          <Routes>
            <Route path="/" element={<BrachotPage onNavigate={(p: any) => navigate(`/${p}`)} />} />
            <Route path="/shabbat" element={<ShabbatPage />} />
            <Route path="/siddur" element={<SiddurPage />} />
            <Route path="/halacha" element={<HalachaPage />} />
            <Route path="/tracker" element={<MitzvahTracker />} />
      ...
      <Footer onNavClick={(p: any) => navigate(`/${p}`)} />
      <AccessibilityMenu />
    </div>
  );
};
